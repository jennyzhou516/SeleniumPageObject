	...
	private void verifyMessageError(HashMap<String, String> fieldData) {
		if (fieldData.get("LastNameMesEr") != "") {
			...
			if (fieldData.get("FirstNameMesEr") != "") {
				elementTextEqual("First Name message error", signUPPage.errormsg_FirstName,
						fieldData.get("FirstNameMesEr"));
			}
		}
		if (fieldData.get("UserNameMesEr") != "") {
			elementTextEqual("User name message error", signUPPage.errormsg_GmailAddress,
					fieldData.get("UserNameMesEr"));
		}
	}
